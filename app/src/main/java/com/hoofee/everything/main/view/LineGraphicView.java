...
public class LineGraphicView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        mPaint.setColor(res.getColor(R.color.theme_color));
//
//		drawAllXLine(canvas);
//		// 画直线（纵向）
//		drawAllYLine(canvas);
        ...
        for (int i = 0; i < yRawData.size(); i++) {
            //分别加入每一个点，每一个点是平均值
            xList.add(ConvertUtils.getWidth(mContext) / yRawData.size() * i + ConvertUtils.getWidth(mContext) / yRawData.size() / 2);
//            xList.add(blwidh + (canvasWidth - blwidh) / yRawData.size() * i);
        }
        mPoints = getPoints();

        mPaint.setColor(res.getColor(R.color.red));
        mPaint.setStrokeWidth(dip2px(3.0f));
...
//					canvas);// X坐标
    ...
    private void drawText(String text, int x, int y, Canvas canvas) {
        ...
        p.setColor(res.getColor(R.color.gray));
